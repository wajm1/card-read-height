...
def check_reader() -> tuple[bool, str]:
    ...
    if not os.path.isfile(RRM_CLI):
        return False, f"RRMTool_CLI not found at {RRM_CLI}. {config.RRM_CLI_HELP}"
    out = run_cli("-about")
...
def configure_reader_for_card(
    ...
    *,
    ...
    def log(msg, tag="info"):
        # Prefer the caller's logger. ``print`` is a common mistake for log_fn
        # (it accepts the tag as a second argument and prints "… error").
