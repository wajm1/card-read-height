...
# Persistent reader-calibration store (remembers the card tap location across
# GUI restarts). Guarded so a missing module can never stop the GUI launching.
try:
    from persistence.calibration_store import (
        load_calibration as _load_calib,
        save_calibration as _save_calib,
    )
except Exception:            # pragma: no cover
    _load_calib = None
    _save_calib = None
...
from constants import (
    ...
    READ_ANGLES, joint_limit_issues,
    DESCENT_PRESETS, DEFAULT_PRESET,
    FIXED_ZONE_TAPS,
...
class App:
    ...
    def __init__(self, root):
        ...
        self._calib_saved_meta = None       # metadata of the last loaded saved calibration
        ...
        # Remember the calibration marked for this reader in a previous session
        # so the card tap location persists across GUI restarts (recalibrate to
        # change it). Sets self._calib_* — does not touch UI (not built yet).
        self._load_saved_calibration(self.reader_type.get())
    ...
    def _on_reader_selected(self, _e=None):
        ...
        # A calibration belongs to the reader it was marked on. Load this
        # reader's saved calibration if we have one; otherwise clear so a
        # different reader's values can't carry over.
        if self._load_saved_calibration(self.reader_type.get()):
            self.set_status(
                "Loaded saved calibration for {} — reader top {:.1f} mm. "
                "Recalibrate to change it.".format(
                    self.reader_type.get(), self._calib_reader_height or 0.0))
        else:
            self.set_status(
                "No saved calibration for {} — run CALIBRATE READER.".format(
                    self.reader_type.get()))

    def _load_saved_calibration(self, reader_model):
        """Populate calibration state from the on-disk store for this reader.

        Returns True when a saved calibration was found and loaded. When none
        exists (or the store module is unavailable) the calibration is cleared
        so a different reader's values never carry over. Touches only
        ``self._calib_*`` attributes — safe to call before the UI is built.
        """
        self._calib_reader_height = None
        self._calib_reader_floor_above_table = None
        self._calib_staging_pose = None
        self._calib_saved_meta = None
        if _load_calib is None:
            return False
        try:
            saved = _load_calib(reader_model)
        except Exception as e:          # pragma: no cover - never block launch
            print(">> Calibration load failed: {}".format(e))
            saved = None
        if not saved:
            return False
        self._calib_reader_height = saved.get("reader_height_mm")
        self._calib_reader_floor_above_table = saved.get("reader_floor_above_table_mm")
        self._calib_staging_pose = saved.get("staging_pose_deg")
        self._calib_saved_meta = saved
        print(">> Loaded saved calibration for {}: reader top {:.1f} mm "
              "(marked {}).".format(
                  reader_model, self._calib_reader_height or 0.0,
                  saved.get("marked_at") or "?"))
        return True

    def _save_current_calibration(self):
        """Persist the just-captured calibration to disk (keyed by reader model).

        Returns True on a successful write. A missing store module or an
        incomplete capture is a no-op returning False (the calibration still
        works for the current session).
        """
        if _save_calib is None:
            return False
        if (self._calib_reader_height is None
                or self._calib_reader_floor_above_table is None):
            return False
        try:
            return bool(_save_calib(
                self.reader_type.get(),
                reader_height_mm=self._calib_reader_height,
                reader_floor_above_table_mm=self._calib_reader_floor_above_table,
                staging_pose_deg=self._calib_staging_pose,
            ))
        except Exception as e:
            print(">> Calibration save failed: {}".format(e))
            return False

    def _effective_reader_height(self):
        """Reader height (mm, table-to-top) from MARK READER TOP.

        Prefers a calibration captured/loaded this session; optional legacy
        fallback is card_readers.json via _reader_height_for (None when that
        file is absent).
        """
    ...
    def show_calibrator(self):
        ...
        # capture (angle + height, one button). If a calibration was already
        # loaded from disk for this reader, show it so the operator knows the
        # tap location is remembered and can MARK again to change it.
        if self._calib_reader_height is not None:
            _calib_init_txt = (
                "Saved calibration loaded — reader top {:.1f} mm{}.\n"
                "Jog and MARK to recalibrate.".format(
                    self._calib_reader_height,
                    " (marked {})".format(self._calib_saved_meta.get("marked_at"))
                    if isinstance(self._calib_saved_meta, dict)
                    and self._calib_saved_meta.get("marked_at") else ""))
        else:
            _calib_init_txt = "Reader: (not captured)"
        self.calib_height_lbl = tk.Label(lp, text=_calib_init_txt,
        ...
        # XY pad — X (forward/back) inverted for operator feel. Y (left/right)
        # swapped 2026-07-30 so ◀ moves the arm left and ▶ right (was reversed);
        # the −Y/+Y labels now match the actual axis sign each button commands.
    ...
    def _calib_bind_keys(self):
        ...
        binds = {
            # Forward/back (X) inverted for operator feel. Left/Right (Y) swapped
            # 2026-07-30 so ← jogs the arm left and → right (were reversed); these
            # now match the ◀/▶ on-screen buttons.
    ...
    def _calib_finish_mark(self, pose, reader_h, floor):
        ...
        # Persist so the tap location is remembered after the GUI is closed and
        # reopened. Recalibrating (MARK again) overwrites the saved value.
        saved = self._save_current_calibration()
        persist_note = ("saved — remembered on next launch" if saved
                        else "active this session (not saved to disk)")
        self.calib_height_lbl.config(
            text="Captured — height {:.1f} mm, floor set here, approach angle "
                 "updated ({}).\nStaging pose: {}".format(reader_h, persist_note, pose),
            fg=BRAND['green'])
        self._calib_set_status(
            "Reader captured: position + height ({}).".format(persist_note),
            BRAND['green'])
