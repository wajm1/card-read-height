...
class GuiRobot(RobotMain):
    ...
    # =====================================================================
    # DEADZONE TEST  (normal reader config + slow ascent from reader top)
    # =====================================================================
    ...
    def _deadzone_measure_side(self, idx, card_name, barcode, side_label):
        ...
        # Reader was configured normally (same as the other tests), so there
        # is no continuous-mode lockout to restore here.

    def run_deadzone(self):
        """Deadzone run: pick → scan/config reader (normal) → touch reader → slow ascent."""
        try:
            print('>> Homing (fast)...')
            if not self._move_joint(config.HOME_ANGLE, 'home'):
                return
            print(
                '>> Home reached. Deadzone on {} card(s) — normal reader config, '
            ...
            for i in range(self.cfg_cycles):
                ...
                # Barcode + reader config — same normal config as the other
                # tests (read-height / tap-and-go).
                name, barcode = self._goto_scan_barcode(i + 1)
                if barcode is None:
                    print('>> No barcode — skipping, releasing card.')
                    self._progress(i + 1, self.cfg_cycles, "Releasing card")
                    self._release_card()
