...
# ``msvcrt`` (the Windows-only Q-to-stop key listener) is imported lazily inside
# _listen_for_stop() so this motion library can be imported and unit-tested on
# non-Windows hosts. On Windows behavior is unchanged.
...
class RobotMain:
    ...
    def _listen_for_stop(self):
        try:
            ...
            # Q-to-stop is a Windows console feature; skip gracefully elsewhere.
            return
    ...
    def _descend_until_read(
        ...
        # Extra hold-and-listen once the card is all the way down at the reader
        # top (for low-profile readers that only read when touching).
        if floor_dwell_s is None:
            floor_dwell_s = getattr(config, "READER_DESCENT_FLOOR_DWELL_S", 0.5)
        if start_lift_mm > 0:
        ...
        try:
            ...
            while True:
                if not self.is_alive:
                    break
                if dropped >= max_drop:
                    # At the lowest allowed point — the card is at the reader top
                    # (touching, after MARK READER TOP). Low-profile readers often
                    # only read when essentially touching and need a moment to
                    # fire, so HOLD here and keep listening before concluding "no
                    # read". Only costs time when nothing has read yet.
                    if not read_found and floor_dwell_s > 0 and self.is_alive:
                        print(
                            f">> At floor — holding {floor_dwell_s:.2f}s for a read "
                            f"(card on reader)..."
                        )
                        deadline = time.monotonic() + floor_dwell_s
                        while time.monotonic() < deadline and self.is_alive:
                            if listener.read_detected():
                                break
                            time.sleep(0.02)
                        if listener.read_detected():
                            ...
                            print(
                                f">> Card READ at floor after {floor_dwell_s:.2f}s hold."
                            )
                            break
