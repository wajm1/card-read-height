...
# NOTE: ``keyboard`` is imported lazily inside BarcodeListener.start()/stop()
# rather than at module load. This decouples the card-lookup / AllCards.csv /
# CSV-baseline logic (which has no hardware dependency) from the global keyboard
# hook, so those pure functions can be imported and unit-tested on any machine
# without the ``keyboard`` package or a wedge scanner present.
...
def register_tk_text_input(tk_root, widget) -> None:
    """Mark a GUI text field so the global wedge hook never swallows typed keys.

    The barcode/credential reader is a keyboard wedge, captured by a global
    ``keyboard`` hook that runs on its OWN thread. From that thread a cross-thread
    Tk focus query (``focus_get``) is unsafe and on Windows simply fails, so the
    only reliable "the operator is typing, pass keys through" signal is a plain
    attribute on the root: ``tk_root._pass_keys_to_gui``.

    Earlier this flag was a bare boolean toggled by each widget's FocusIn/FocusOut.
    When focus moved between two registered fields, a late FocusOut could fire
    AFTER the new field's FocusIn and wrongly clear the flag — leaving it False
    while a field was focused, which made the hook drop the first characters
    typed (notably a leading 'A'/'B', mistaken for a barcode prefix). See the
    Comment field "can't type" bug.

    Fix: track WHICH registered widget currently holds focus
    (``tk_root._focused_text_widget``) and derive the flag from it. A field only
    clears the flag if it is still the focused one, so the swap race can't strand
    it False. We also set focus on a raw ``<Button-1>`` click, so a click-then-type
    sequence is honored even if FocusIn is flaky in the operator's environment.
    """
    if tk_root is None:
        return
    if not hasattr(tk_root, "_focused_text_widget"):
        tk_root._focused_text_widget = None
    tk_root._pass_keys_to_gui = False

    def _set_focused(w):
        tk_root._focused_text_widget = w
        tk_root._pass_keys_to_gui = w is not None
    ...
    def on_focus_out(_event=None):
        # Only clear when WE are the field losing focus (guards the swap race).
        if getattr(tk_root, "_focused_text_widget", None) is widget:
            _set_focused(None)
...
class BarcodeListener:
    ...
    def start(self):
        import keyboard          # lazy: only needed when actually listening
        if self.active:
            return
        self.buf = ""
        self._shift = False
